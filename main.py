...
def update():
    ...
    if oneSecTimer()  :
        ...
        inflation = 0
        if bought >0:
            deflation = (random.randrange(0 - bought*40, 100) * 0.01)
        if sold >0:
            inflation = (random.randrange(0 , 0 +  sold*40) * 0.01)
    ...
    if pyxel.btnp(pyxel.KEY_C): #acheter
        if money > stock_price :
            ...
            #stock_price *= (random.randrange(70,100) * 0.01)
...
def draw():
    ...
    pyxel.text(115,5,"raise " + str(goal) ,7)
    pyxel.text(115,105," n: sell",7)
    pyxel.text(115,95," c: buy",7)
